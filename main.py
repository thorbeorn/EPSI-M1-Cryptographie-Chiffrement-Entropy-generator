...
from graphique import nuage_points_from_values, nuage_points_tries_from_values
...
create_roulette(generated_hex[0])

seed = int(generated_hex[0], 16)
random.seed(seed)
values = [random.randint(0, 36) for _ in range(1000)]
print(f"\nGénération du nuage de points...")
nuage_points_from_values(values)
nuage_points_tries_from_values(values)
print(f"\n{'='*60}")
