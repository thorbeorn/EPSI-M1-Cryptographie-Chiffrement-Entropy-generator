...
from graphique import nuage_points_from_values, nuage_points_tries_from_values, frequence_sortie_from_values
...
create_roulette(generated_hex[len(generated_hex)-1])

print(f"\nGénération du nuage de points...")
nuage_points_from_values(generated_numbers)
nuage_points_tries_from_values(generated_numbers)
frequence_sortie_from_values(generated_numbers)
print(f"\n{'='*60}")
print(f"NUAGE DE {NOMBRE_GENERATIONS} POINTS GÉNÉRÉE")
print(f"{'='*60}")
print(f"Nuage de points sauvegardé dans: {OUTPUT['nuage_points_graph']}\n")
print(f"Nuage de points trié sauvegardé dans: {OUTPUT['nuage_points_graph_trie']}\n")
print(f"bar sauvegardé dans: {OUTPUT['frequencie_graph']}\n")
